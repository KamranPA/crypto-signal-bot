...
def apply_trend_strategy(df, adx_threshold=20, volume_ratio_threshold=1.2):
    ...
    """
    if len(df) < 50:
        print("⚠️ trend_strategy: تعداد کندل‌ها کمتر از 50 است")
        return None

    print(f"📊 trend_strategy: بررسی {len(df)} کندل")
    ...
    print(f"📈 وضعیت EMA21:")
    print(f"   قیمت فعلی: {current_close:.6f} | قبلی: {prev_close:.6f}")
    print(f"   EMA فعلی: {current_ema:.6f} | قبلی: {prev_ema:.6f}")
    ...
    if current_close > current_ema and prev_close <= prev_ema:
        ...
        print(f"✅ سیگنال خرید تولید شد: ورود={entry:.6f}, SL={sl:.6f}, TP={tp:.6f}")
        return {
            'signal': 'BUY',
            'entry': entry,
            'stop_loss': sl,
    ...
    # --- 8. بررسی عبور از EMA (فروش)
        ...
        print(f"✅ سیگنال فروش تولید شد: ورود={entry:.6f}, SL={sl:.6f}, TP={tp:.6f}")
        return {
            'signal': 'SELL',
            'entry': entry,
            'stop_loss': sl,
