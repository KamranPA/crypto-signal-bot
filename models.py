...
def train_lstm(X_train, y_train, input_shape):
    model = Sequential([
        ...
        Dropout(0.2),
        Dense(25),
        Dense(3, activation='softmax')  # 3 کلاس: -1, 0, 1
    ])
...
# شبیه‌سازی تحلیل احساسات (در نسخه واقعی از API خبری استفاده می‌شود)
def get_sentiment_score():
    # در عمل از CryptoPanic یا Twitter API استفاده می‌شود
    import random
    return random.uniform(-1, 1)  # -1 (منفی)، +1 (مثبت)
