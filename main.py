...
def load_signals():
    if os.path.exists(SIGNALS_FILE):
        try:
            ...
            print(f"❌ خطا در خواندن signals.json: {e}")
            return {}
    return {}
...
def save_signals(data):
    try:
        ...
        print(f"❌ خطا در ذخیره signals.json: {e}")
...
# ——————————————————————————
# توابع تحلیلی ایمن (با مدیریت خطا و NaN)
# ——————————————————————————
def ema(data, period):
    try:
        series = pd.Series(data)
        ema_vals = series.ewm(span=period, adjust=False).mean().values
        return np.nan_to_num(ema_vals, nan=0.0)
    except Exception as e:
        print(f"❌ خطای EMA: {e}")
...
def rsi(data, period=14):
    try:
        if len(data) < period + 1:
            return np.array([50.0] * len(data))  # مقدار پیش‌فرض
        delta = np.diff(data)
        gain = np.where(delta > 0, delta, 0)
        loss = np.where(delta < 0, -delta, 0)
        avg_gain = pd.Series(gain).rolling(period).mean()
        avg_loss = pd.Series(loss).rolling(period).mean()
        rs = avg_gain / (avg_loss + 1e-8)  # جلوگیری از تقسیم بر صفر
        rsi_vals = 100 - (100 / (1 + rs))
        return np.nan_to_num(rsi_vals, nan=50.0)  # جایگزینی NaN با 50
    except Exception as e:
        print(f"❌ خطای RSI: {e}")
...
def find_swing_low(low, window=5):
    try:
        ...
        print(f"❌ خطای یافتن Swing Low: {e}")
        return None
...
def find_swing_high(high, window=5):
    try:
        ...
        print(f"❌ خطای یافتن Swing High: {e}")
        return None
...
# ——————————————————————————
# تشخیص سیگنال خرید (BUY) — با دیباگ کامل
# ——————————————————————————
def check_buy_signal(df):
    try:
        ...
        if np.isnan(rsi_vals[-1]) or np.isnan(rsi_vals[-2]) or np.isnan(rsi_vals[-3]):
            print("❌ [BUY] RSI: مقادیر NaN دارد")
            return None
        ...
        try:
            ...
            print("❌ [BUY] داده کافی برای بررسی الگو نیست")
            return None
        ...
        pattern_ok = is_hammer or is_bearish_engulfing
        ...
        print(f"💥 خطای داخلی در check_buy_signal: {e}")
        return None

# ——————————————————————————
# تشخیص سیگنال فروش (SELL) — با دیباگ کامل
# ——————————————————————————
def check_sell_signal(df):
    try:
        ...
        if np.isnan(rsi_vals[-1]) or np.isnan(rsi_vals[-2]) or np.isnan(rsi_vals[-3]):
            print("❌ [SELL] RSI: مقادیر NaN دارد")
            return None
        ...
        try:
            ...
            print("❌ [SELL] داده کافی برای بررسی الگو نیست")
            return None
        ...
        print(f"💥 خطای داخلی در check_sell_signal: {e}")
        return None

# ——————————————————————————
# تابع اصلی — با try-except کامل
# ——————————————————————————
def main():
    ...
    try:
        ...
        try:
            ...
            error_msg = f"❌ خطای دریافت داده از KuCoin: {e}"
            print(error_msg)
        ...
        if df.empty or len(df) < 200:
            print("❌ داده نامعتبر یا کمتر از 200 کندل")
            return
        ...
        if time_since_close < 60:
            # ✅ ارسال پیام "سیستم فعال" هر 55 دقیقه
...
⏱ آخرین بررسی: {now_dt.strftime('%Y-%m-%d %H:%M:%S UTC')}
        ...
        error_msg = f"💥 خطای ناگهانی در سیستم:\n{e}"
        print(error_msg)
        send_telegram_message(TELEGRAM_TOKEN, TELEGRAM_CHAT_ID, error_msg)
